...
layout(location=8) in mat3 tangentBasis;
...
void main()
{
	...
	color = vec4(1.0);
	//return;
	vec3 albedo = texture(albedoTexture, fuv).rgb;
	float metalness = texture(metalnessTexture, fuv).r;
	float roughness = texture(roughnessTexture, fuv).r;
	color = vec4(albedo.rgb,1.0);
	//color = vec4(fuv.xy,0.0,1.0);
	//return;

	// Outgoing light direction (vector from world-space fragment position to the "eye").
	vec3 Lo = normalize(camPos - fpos);
	...
	*/
	...
	vec3 ambientLighting;
	{
		...
		color = vec4(specularIrradiance.xyz,1.0);
		return;
