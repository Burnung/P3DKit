...
#include "products/face3D/Face3dRTSolveCeres.h"
...
public:
	...
	virtual void onInit() {
		...
		face3DRtSolver = std::make_shared<Face3dRTSolveCeres>();
		face3DRtSolver->init(memDataStore);
	...
	void runFaceArith(cv::Mat& srcImg) {
		...
		if (!faces.empty()){
			...
			if(faceRtTracker == nullptr)
				faceRtTracker = std::make_shared<Face3dRtTrackInfo>();

			//void Face3dRTSolveCeres::fit_pose(pImage image, int image_width, int image_height, float fov,
			//	int track_id, std::vector<float>& face_landmarks,
			//std::shared_ptr<Face3dResult>& ret, std::shared_ptr<Face3dRtTrackInfo> track){
			face3DRtSolver->fit_pose(img,img->width(), img->height(), 1.0, 0,faceLds, faceRet, faceRtTracker);
