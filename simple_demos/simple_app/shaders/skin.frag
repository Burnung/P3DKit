...
float hardShadow(sampler2D _sampler, vec4 _shadowCoord, float _bias)
{
	...
	return step(texCoord.z-_bias, texture(_sampler, texCoord.xy).x );
}
...
void main(){
	...
	vec3 SSSPowerMap = texture2D(s_texSSSPower, fuv).xyz;

	vec3 AlbedoMap = texture2D(s_texAlbedo, fuv).xyz;
	//outColor = vec4(AlbedoMap.rgb,1.0);
	//outColor = vec4(ScreenPos.xy,0.0,1.0);
	//outColor = vec4(ScreenPos.xy,0.0,1.0);
	
	vec3 AOMap = texture2D(s_texAO, fuv).xyz;
	float AO = pow(AOMap.x,AOPower);
	vec3 BaseNormalMap = texture2D(s_texNormal, fuv).xyz;
	vec3 HightLightMap = texture2D(s_texGloss, fuv).xyz;
	...
	outColor = vec4(AlbedoMap.xyz,1.0);
	return;
	...
	vec3 DetailNormalMap1 = texture2D(s_texDetailNormal, fuv*DetailTextureScale).xyz;
	vec3 DetailNormalMap2 = texture2D(s_texDetailNormal, fuv*DetailTextureScale*0.5).xyz;
	...
	{//expression
		...
		//expression_albedo
		AlbedoMap = expression_albedo(AlbedoMap, fuv, brow_mask1, brow_mask2, cheek_mask, nose_mask, mouth_mask1, mouth_mask2);

		//expression_spec
		SpecularPower = expression_spec(SpecularPower, fuv, brow_mask1, brow_mask2, cheek_mask, nose_mask, mouth_mask1, mouth_mask2);

		//expression_normal
		vec4 outResult = expression_normal(NormalMapDiffuse.xy,NormalMapSpecular.xy, fuv, brow_mask1, brow_mask2, cheek_mask, nose_mask, mouth_mask1, mouth_mask2);
	...
	vec2 cellLumi = cellular2x2x2(vec3(fuv*1000.0,0.0));
	...
	vec3 Radiance1    = (textureLod(s_rttCube, RadianceDirection.xyz, MipLevel1).xyz);
	vec3 Radiance2    = (textureLod(s_rttCube, RadianceDirection.xyz, MipLevel2).xyz);
	vec3 RadianceWet  = (textureLod(s_rttCube, RadianceDirection.xyz, 2.0).xyz);
